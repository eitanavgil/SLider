...
import { CheckTypes } from "../components/GameOptions/GameOptions";
...
// compare 2 and check
export const check = (
  board1: boardItemData[][],
  board2: boardItemData[][],
  checkPolicy?: CheckTypes
): boolean => {
  const cleanBoard1 = getValues(board1);
  const cleanBoard2 = getValues(board2);
  switch (checkPolicy) {
    case CheckTypes.center:
    // here implement later center-check
    default:
...
export const getCoordinatesByItem = (
  board: boardItemData[][],
  item: boardItemData
) => {
  for (let y = 0; y < board.length; y++) {
...
export const getItemByCoordinates = (
  x: number,
  y: number,
  board: boardItemData[][]
): boardItemData | null => {
...
///////////////////////////
export const getItemsAbove = (
  flattenBoard: boardItemData[],
  zeroCoordinates: coordinates
): boardItemData[] => {
  return flattenBoard.filter((item: boardItemData) => {
    return (
      item.x !== undefined &&
      item.x === zeroCoordinates.x &&
      item.y !== undefined &&
      item.y < zeroCoordinates.y
    );
  });
};
export const getItemsBelow = (
  flattenBoard: boardItemData[],
  zeroCoordinates: coordinates
): boardItemData[] => {
  return flattenBoard.filter((item: boardItemData) => {
...
export const multiMove = (
  board: boardItemData[][],
  item: boardItemData,
  zeroCoordinates: coordinates,
  zeroItem: boardItemData
) => {
  const copy = cloneDeep(board);
  ...
  switch (item.allowedDirection) {
    case directions.RIGHT:
      ...
      const itemsToLeft = reverse(
        getItemsToLeft(flattenArray, zeroCoordinates)
      );
